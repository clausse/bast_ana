% Script to analyze butterfly from E200 2013 data with Butterfly on CELOSS camera
...
data_set = 'E200_10991';
do_save = 0;
save_path = ['C:/Users/Bastien/Desktop/PRE/Matlab/2013_E200_Data_Analysis/' day '/'];
...
%% ZOOM SETTINGS
    ...
    nb_slice = 25;  % it represents the number of slices, and so the number of points you will use for the fit
...
%%  SLICES AND SIGMA  
    ...
    CELOSS.reduc = reduc(CELOSS.slice, nb_slice); % it defines the line-outs that will be plotted.
    ...
    A = sigmacarre(CELOSS.slice', CELOSS.yyzoom, nb_slice); % A is a vector with the different values of sigma squared with the desired zoom.
   
    p = polyfit(yparabole, A,order_fit); % Fit calculation
    x2 = yparabole;
    y2 = polyval(p,x2);
...
LeftAxes2 = axes('position', [0.37, 0.065, 0.22, 0.39]);    % CELOSS.slice display : desired zoom of the butterfly's parabola
...
axes('position', [0.7, 0.565, 0.22, 0.39])  % Parabola display: CELOSS.slice calculations and parabolic fit
        hold all;
